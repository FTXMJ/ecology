...
//生态仓库释放　－　团队收益  --  直推收益
func ProducerEcology(o orm.Ormer, users models.User, order_id string) {
	Worker(o, users, order_id)
}
...
// 工作　函数
func Worker(o orm.Ormer, user models.User, order_id string) error {
	team_a_bouns := 0.0
	ziyou_a_bouns := 0.0
	zhitui_a_bouns := 0.0
	o.Begin()
...
// 给失败的用户　添加失败的任务记录表
func CreateErrUserTxList(user models.User) {
	o := db.NewEcologyOrm()
	//任务表 USDD  铸币记录
	order_id := utils.TimeUUID()
	blo_txid_dcmt := models.TxIdList{
